...
class disjoint_set2 : public disjoint_set1{
public:
    disjoint_set2(unsigned n):disjoint_set1(n) { }
    int Find(int x) noexcept{ return p[x]; }
...
void Kruskal_array(vector<vector<Edge>> *sg, int n)
{   disjoint_set2 set(n);
...
void Kruskal_tree(vector<vector<Edge>> *sg, int n)
{   disjoint_set1 set(n);
...
{   Random::Rand_Init();
    ...
    cin >> c;
    cout << "On array or tree? (0 - array, 1 - tree): ";
    cin >> choice;
    cout << "Running..." << endl;
...
{   int n = 1 << i;
    ...
    {   for(auto& v: sortedEdges) // Reserve memory
        ...
        if(choice){
            t1 = CoolClock::now();
            Kruskal_tree(&sortedEdges, n);
        }else{
            t1 = CoolClock::now();
            Kruskal_array(&sortedEdges, n);
        }
        t2 = CoolClock::now();
        file << ToMilliseconds(t2 - t1).count()
             << " ms for Kruskal on " << (choice ? "tree" : "array") << endl;
